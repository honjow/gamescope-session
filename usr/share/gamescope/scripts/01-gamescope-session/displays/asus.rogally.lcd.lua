...
gamescope.config.known_displays.rogally_lcd = {
    ...
    matches = function(display)
        local lcd_types = {
            ...
            { vendor = "BOE", data_string = "TS070FHM-LU0", name = "ROG Ally X" }, -- BOE-TS070FHM-LU0
        }

        for index, value in ipairs(lcd_types) do
            if (value.vendor == display.vendor) then 
                if (value.model and value.model == display.model)
                  or (value.data_string and value.data_string == display.data_string) then
                    info("["..value.name.."] Matched vendor: "..value.vendor.." model: "..(value.model or value.data_string).." product: "..display.product)
                    return 5100
                end
            end
        end
